...
class PythonExecutorTool:
    ...
    def __init__(self):
        ...
        # 自动安装包功能已启用，不再使用白名单
        logger.info("PythonExecutorTool: Automatic package installation is enabled for any missing package.")
        ...
        try:
            ...
            if (
                ...
                if missing_match:
                    ...
                    logger.info(f"Attempting to auto-install missing package: {pkg}")
                    install_res = await self.install_package(pkg)
                    if install_res.get("success"):
                        logger.info(f"Successfully auto-installed {pkg}. Retrying code execution.")
                        result = await _run_code()
                        result["auto_installed"] = pkg
                    else:
                        logger.error(f"Failed to auto-install {pkg}. Error: {install_res.get('error')}")
