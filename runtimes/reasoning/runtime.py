...
from .tools import get_python_executor_tool, deep_research_tool, get_browser_use_tool
...
class ReasoningRuntime(RuntimeInterface):
    ...
    def capabilities(self) -> list:
        return ['deep_research', 'python_executor', 'browser_use']
    ...
    async def execute(self, task: TaskSpec) -> TrajectoryResult:
        ...
        # 添加重复检测和限制
        deep_research_queries = []  # 跟踪深度研究查询
        max_deep_research_calls = 3  # 最多允许3次深度研究调用
        ...
        for step_id in range(1, task.max_steps + 1):
            ...
            for attempt in range(max_retries + 1):
                ...
                if action == 'deep_research' and tool_name == 'deep_research':
                    ...
                    if deep_research_count > max_deep_research_calls:
                        logger.warning(f"深度研究调用次数达到限制 ({max_deep_research_calls})，跳过此次调用")
                        tool_success = False
                        current_attempt_err_type = ErrorType.SYSTEM_ERROR
                        current_attempt_err_msg = f"深度研究调用次数超过限制 ({max_deep_research_calls})"
                        observation = f"错误：深度研究调用次数已达到限制 ({max_deep_research_calls})"
                        action_type = ActionType.TOOL_CALL
                    else:
                        # 检查查询重复性
                        is_duplicate = False
                        for prev_query in deep_research_queries:
                            # 简单的相似性检测
                            if self._queries_similar(query, prev_query):
                                is_duplicate = True
                                logger.warning(f"检测到重复的深度研究查询: '{query}' 与 '{prev_query}' 相似")
                                break
                        
                        if is_duplicate:
                            tool_success = False
                            current_attempt_err_type = ErrorType.SYSTEM_ERROR
                            current_attempt_err_msg = "检测到重复的深度研究查询"
                            observation = "错误：检测到重复的深度研究查询，请尝试不同的研究角度"
                            action_type = ActionType.TOOL_CALL
                        else:
                            # 记录查询并执行
                            deep_research_queries.append(query)
                            ...
                            logger.info(f"深度研究调用 {deep_research_count}/{max_deep_research_calls}: {query[:100]}...")
                    ...
                    last_full_tool_output = observation_json
                    action_type = ActionType.TOOL_CALL
                elif action == 'browser_task' and tool_name == 'browser_use':
                    task_desc = params.get('task_description', '')
                    logger.debug(f"Attempt {attempt + 1}: BrowserUse execute - Task: {task_desc}")
                    browser_tool = get_browser_use_tool()
                    res = await browser_tool.execute(task_desc)
                    tool_success = res.get('success', False)
                    observation_json = json.dumps(res, ensure_ascii=False)
                    observation = truncate_text(observation_json, 2000)
                    last_full_tool_output = observation_json
                    action_type = ActionType.TOOL_CALL
                    if tool_success and 'final_result' in res:
                        current_outputs.append(res['final_result'])
                elif action == 'complete_task':
                    summary = await self.client.generate_task_summary(
                        task.description, [s.to_dict() for s in steps], current_outputs
                    )
            ...
            # 如果工具执行失败，将结构化的错误信息添加到观察中，以便LLM更好地理解
            if not tool_success:
                error_feedback = {
                    "error_summary": f"Tool action '{action}' failed.",
                    "error_details": current_attempt_err_msg,
                    "error_type": current_attempt_err_type.value if current_attempt_err_type else "UNKNOWN_ERROR",
                    "suggestion": "Please analyze the error and try a different approach or tool."
                }
                # 更新最后一步的观察结果
                steps[-1].observation = json.dumps(error_feedback, ensure_ascii=False)
            # 结构化日志：StepLog
            logger.debug(json.dumps({
                "step_id": step_id,
                "action_type": action_type.name if hasattr(action_type, 'name') else action_type,
            ...
            # 如果达到深度研究限制，且至少完成了一次深度研究，强制判断为完成
            if deep_research_count >= max_deep_research_calls and deep_research_queries:
                logger.info(f"达到深度研究限制 ({max_deep_research_calls})，且已完成研究，判断任务完成")
                success = True
                break
    ...
    async def cleanup(self):
        ...
        browser_use_tool = get_browser_use_tool()
        if hasattr(browser_use_tool, 'close'):
            await browser_use_tool.close()
