...
async def web_research(state: WebSearchState, config: Dict[str, Any] = None) -> OverallState:
    ...
    try:
        ...
        import time as time_module  # 确保正确导入
        loop_start_time = time_module.time() # 默认值
        ...
        # 添加超时保护的搜索执行
        import asyncio
        import time as time_module  # 明确导入 time 模块，避免作用域问题
        ...
        try:
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                response = await asyncio.get_event_loop().run_in_executor(None, lambda: future.result(timeout=20))
        except Exception as e:
...
async def reflection(state: OverallState, config: Dict[str, Any] = None) -> ReflectionState:
    ...
    try:
        ...
        import time as time_module  # 确保正确导入
        loop_start_time = state.get("loop_start_time", time_module.time())
        ...
        # 生成反思提示词
        research_topic = get_research_topic(state["messages"])
        # 安全地处理 web_research_result，确保是字符串
...
def evaluate_research(state: ReflectionState, config: Dict[str, Any] = None) -> Union[str, List[Send]]:
    ...
    try:
        ...
        # 检查总时间限制（第一优先级）
        import time as time_module  # 确保正确导入
        start_time = state.get("start_time", time_module.time())
        elapsed_time = time_module.time() - start_time
        MAX_TOTAL_TIME = 300  # 5分钟强制退出
        
        logger.info(f"评估研究进度 - 循环: {current_loop}/{max_research_loops}, 查询: {total_queries}/{max_total_queries}, 充分性: {state['is_sufficient']}, 已用时: {elapsed_time:.1f}s")
        
        # 强制退出条件 1: 达到最大时间限制
        if elapsed_time > MAX_TOTAL_TIME:
            logger.warning(f"达到最大时间限制{MAX_TOTAL_TIME}秒，强制结束研究")
            if "research_tracker" in state:
                state["research_tracker"].record_forced_exit(f"达到最大时间限制{MAX_TOTAL_TIME}秒")
