...
class TestValidationCritic:
    ...
    @pytest.mark.asyncio
    async def test_tool_correction_suggestion(self, validation_critic):
        """测试工具修正建议"""
        # 测试不存在的工具ID
    ...
    @pytest.mark.asyncio
    async def test_action_correction_mappings(self, validation_critic):
        """测试动作修正映射"""
        # 测试已知工具的动作修正
        corrected = await validation_critic._suggest_action_correction("mcp-deepsearch", "invalid_action")
        assert corrected in ["research", "quick_research", "comprehensive_research"]
        
        corrected = await validation_critic._suggest_action_correction("microsandbox-mcp-server", "run")
