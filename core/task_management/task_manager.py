...
from core.interfaces.task_interfaces import TaskSpec, TrajectoryResult
...
def get_runtime(task_type: str):
    ...
    if task_type == 'reasoning':
        # 优先使用增强版本的推理运行时
        try:
            from runtimes.reasoning import EnhancedReasoningRuntime
        ...
        from runtimes import LightweightSandboxRuntime
        return LightweightSandboxRuntime()
    elif task_type == 'web':
        from runtimes import MemoryControlledWebRuntime
...
async def start_runtime_service(runtime, redis_manager=None):
    ...
    import json
    from core.interfaces.task_interfaces import TaskSpec, TrajectoryResult
    from core.interfaces.common_interfaces import ErrorType
