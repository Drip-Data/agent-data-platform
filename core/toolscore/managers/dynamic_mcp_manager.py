...
from core.toolscore.interfaces.toolscore_interfaces import InstallationResult
from core.toolscore.runners import BaseRunner, ProcessRunner
    ...
    async def _restore_persistent_servers(self):
        ...
        try:
            ...
            with open(config_path, 'r', encoding='utf-8') as f:
                servers_config_raw = json.load(f)

            servers_config = {}
            if isinstance(servers_config_raw, list):
                # 如果是列表，尝试根据 'name' 字段转换为字典
                for item in servers_config_raw:
                    if isinstance(item, dict) and "name" in item:
                        servers_config[item["name"]] = item
                    else:
                        logger.warning(f"跳过无效的MCP服务器配置条目: {item}")
            elif isinstance(servers_config_raw, dict):
                servers_config = servers_config_raw
            else:
                logger.error(f"无效的MCP服务器配置格式: {type(servers_config_raw)}")
                return
            
            restored_count = 0
            for server_name, server_config_item in servers_config.items():
                try:
                    result = await self.runner.install_server(server_config_item)
            ...
            if hasattr(self.runner, 'list_running_servers'):
                ...
                for server_id, server_info in running_servers.items():
                    ...
                    servers_config[server_info.get("name", server_id)] = {
                        ...
                        "server_id": server_id, # 添加 server_id 以便恢复时识别
