...
📌 输出示例2：
- 问题: "请对比分析LoRA方法和AdaLoRA方法的参数量差异是多少倍？"
- 关系应用: "方法A-参数量-对比-方法B"（实体替换：BCA/VeRA→LoRA/AdaLoRA）
- 创造性: ★★★★（需要文献搜索+计算分析）
...
📌 输出示例3：
- 问题: "请分析ResNet架构的核心创新技术是什么，并解释其工作原理？"
...
{{
    "atomic_tasks": [
        {{
            ...
            "task_type": "tool_required|reasoning_only",
            "domain": "任务领域",
            "difficulty": "简单|中等|困难",
...
**🔥 生成要求：**
1. **必须具有创造性**：不是简单复述原始结论
2. **需要推理过程**：问题应该需要Agent通过推理或工具使用才能回答
3. **实体泛化合理**：保持同类型替换，如：公司→公司、方法→方法
4. **关系模式一致**：应用相同的关系结构，但场景要新颖
5. **工具需求明确**：如需要搜索、计算、分析等工具，要明确指出
6. **多步推理优先**：优先生成需要多步推理的复杂问题
7. **避免路径依赖**：生成的任务不应局限于原始轨迹的具体执行路径
