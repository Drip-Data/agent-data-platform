...
判断标准（实用导向）：
- 如果任务涉及搜索、查询、获取实时/外部信息，则需要工具
- 如果任务需要执行代码、计算、文件操作，则需要工具
- 如果任务需要浏览网页、提取数据、自动化操作，则需要工具
- 只有纯概念解释、理论分析、已知知识问答才可能不需要工具
- 涉及具体公司、产品、事件、数据的查询通常需要工具
...
1. 任务是否有一个明确的主要目标？
2. 任务是否可以通过单一工具调用完成？
3. 任务描述中是否不包含明确的多步骤序列（如"首先...然后...最后"）？
4. 任务是否在概念上是一个统一的操作？

实用性考虑：
- "搜索并获取"、"查询并返回"、"计算并显示"等自然表述被视为原子操作
- 工具内部的多个步骤（如搜索+格式化结果）不破坏原子性
- 重点是避免明显需要多个不同工具或明显独立步骤的任务
